...
void tft_init( void ) {
  ...
  *tft_cmd = 0x11;
  ...
  TFT_CMD( 0x29 );
  // Set color mode to 16 bits per pixel.
  TFT_CMD( 0x3A );
  TFT_WR( 0x55 );
  // Display inversion on. When this is off, 0x0000 = white. Call me
  // crazy, but I'm used to '1' meaning 'on' while '0' means 'off'.
  TFT_CMD( 0x21 );
  // Enable 'tearing effect' pin output for vblank only.
  // (Sending 0x01 for the data byte also enables hblank interrupts.)
  TFT_CMD( 0x35 );
  TFT_WR( 0x00 );

  // Read 'display power' to ensure the previous commands worked.
  TFT_CMD( 0x0A );
  // Receive and print a 'dummy byte' from the display.
  volatile uint8_t vv = TFT_RD();
  printf( "Dummy read: 0x%02X\r\n", vv );
  // Receive and print the display's response (should be 0x9C).
  vv = TFT_RD();
  printf( "Display response: 0x%02X\r\n", vv );

  // Set the display drawing area to cover the entire screen.
  tft_draw_fullscreen();
}
...
void tft_px( int x, int y, uint16_t col ) {
  if ( ( x < 0 ) | ( x > 239 ) |
       ( y < 0 ) | ( y > 239 ) | !wrl ) { return; }
  psramh[ x + ( y * 240 ) ] = col;
}

// Draw a horizontal line to the TFT framebuffer.
void tft_hline( int x, int y, int l, uint16_t col ) {
  if ( ( x < 0 ) | ( x > 239 ) |
       ( y < 0 ) | ( y > 239 ) | !wrl ) { return; }
  int len = l;
...
void tft_vline( int x, int y, int l, uint16_t col ) {
  if ( ( x < 0 ) | ( x > 239 ) |
       ( y < 0 ) | ( y > 239 ) | !wrl ) { return; }
  int len = l;
...
void tft_text( int x, int y, const char* str,
  ...
  int offset = 0;
  while( str[ i ] != '\0' ) {
    tft_char( ( x + offset ), y, str[ i ], col, size );
    offset += 6 * size;
    ++i;
  }
}
